...
Key Findings
- AD microglia show reduced branching complexity and altered soma size relative to controls
- Ensemble ML models (Random Forest, Gradient Boosting) classified AD vs. control microglia with ~96% accuracy and AUC = 0.99
- Female microglia exhibited greater morphological simplification than males across nearly all metrics
...
Dependencies
Python 3.x (developed in Google Colaboratory)
neurom==4.0.4
scikit-learn, shap
statsmodels, scipy
...
Reproducibility
All random sampling and cross-validation procedures use a fixed seed (random_state = 42). See code/ for full scripts.
